...
=== ":os-mac: Pääte (MacOS)"
    Helpoin tapa käynnistää *Pääte* on käyttää MacOS:n spotlight-toimintoa. Paina oikean yläkulman suurennuslasia (tai paina yhtä aikaa <nowrap><kbd>:key-cmd: Command</kbd> + <kbd> Välilyönti</kbd></nowrap>), ja kirjoita ilmestyvään hakukenttään Pääte (jos koneesi on englanniksi, niin kirjoita Terminal) ja paina enteriä.
...
!!! info inline end ":os-win: Windows-käyttäjille"
    Tämä sivusto käsittelee paljon *Bashia*, joka on Unix-pohjaisten käyttöjärjestelmien yleisin tulkki. Windows tukee myös sitä, jos asentaa koneelleen **WSL**:n, eli Windows Subsystem for Linuxin. WSL:n asentaminen omalle tietokoneelleen on suositeltavaa, jos aikoo käyttää komentoriviä enemmän.
